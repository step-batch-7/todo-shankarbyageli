class TodoList {
  ...
  nextTaskId(todoId) {
    const todo = this.getTodo(todoId);
    const lastTaskId = todo.tasks[todo.tasks.length - 1].id;
    return +lastTaskId + 1;
  }
  ...
  addItem(todoId, title) {
    const nextTaskId = this.nextTaskId(itemId);
    this.todoList[todoId].tasks[nextTaskId] = {
      title: title,
      status: false
    };
  }
